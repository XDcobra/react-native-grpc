...
- Unary retry / hedging via `createChannel({ retry | hedging })`, `GrpcClient.setRetryPolicy` / `setHedgingPolicy`, and per-RPC overrides (A6-style; mutually exclusive; `GrpcStatusCode` exported)
